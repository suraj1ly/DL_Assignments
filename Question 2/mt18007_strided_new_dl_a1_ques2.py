# -*- coding: utf-8 -*-
"""MT18007_Strided_New_DL_A1_Ques2.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1nKy_hvLgXoIxtxUD8zVylEY9ZdXv4hyD
"""
...
class convTranspose2DBatchNormRelu(nn.Module):
    def __init__(self, in_channels, n_filters, k_size, stride, dilation=1, is_batchnorm=True,):
        super(convTranspose2DBatchNormRelu, self).__init__()

        conv_mod = nn.ConvTranspose2d(int(in_channels), int(n_filters), kernel_size=k_size, stride=stride, dilation=dilation,)
...
class segnetUp1(nn.Module):
    def __init__(self, in_size, out_size):
        ...
        self.deconv1 = convTranspose2DBatchNormRelu(in_size, in_size, 3, 1)
        self.deconv2 = convTranspose2DBatchNormRelu(in_size, out_size, 3, 1)
        # self.conv1 = conv2DBatchNormRelu(in_size, in_size, 3, 1, 1)
        # self.conv2 = conv2DBatchNormRelu(in_size, out_size, 3, 1, 1)

    def forward(self, inputs):
        # outputs = self.unpool(input=inputs, indices=indices, output_size=output_shape)
        # print("up1:", inputs.shape)
        outputs = self.deconv1(inputs)
        outputs = self.deconv1(outputs)
        outputs = self.deconv1(outputs)
...
class segnetUp3(nn.Module):
    ...
    def forward(self, inputs):
        # outputs = self.unpool(input=inputs, indices=indices, output_size=output_shape)
        # print("up3:", inputs.shape)
        outputs = self.deconv1(inputs)
        outputs = self.deconv2(outputs)
        # print("up3:", outputs.shape)
        # outputs = self.conv1(outputs)
        # outputs = self.conv2(outputs)
        # outputs = self.conv3(outputs)
        return outputs
...
class segnetUp5(nn.Module):
    def __init__(self, in_size, out_size):
        ...
        # self.unpool = nn.MaxUnpool2d(2, 2)
        self.deconv1 = convTranspose2DBatchNormRelu(in_size, out_size, 3, 1)
        # self.conv1 = conv2DBatchNormRelu(in_size, in_size, 3, 1, 1)
        # self.conv2 = conv2DBatchNormRelu(in_size, in_size, 3, 1, 1)
        # self.conv3 = conv2DBatchNormRelu(in_size, out_size, 3, 1, 1)
...
num_epochs = 25
pth = '/content/drive/My Drive/Colab Notebooks/DL_StridedModels_A1Q2_v2/'
k = 0
lossArr = []
accArrTrain = []
accArrVal = []
...
print(predVal[testind])
print(valLabels[testind])

plt.imshow(matGT01['gt'][:,:, 128])
plt.show()
